# Largest element in an array

↑ **Parent:** [Functional problem with array as input](functional-problem-with-array-as-input.md)

[https://www.geeksforgeeks.org/program-to-find-largest-element-in-an-array/](https://www.geeksforgeeks.org/program-to-find-largest-element-in-an-array/)

## ↑ Ancestors (9)

1. [Functional problem with array as input](functional-problem-with-array-as-input.md)
2. [Function problem](function-problem.md)
3. [Computational problem](computational-problem.md)
4. [Computer science](computer-science-split.md)
5. [Computer](computer-split.md)
6. [Information technology](information-technology.md)
7. [Area of technology](area-of-technology.md)
8. [Technology](technology-split.md)
9. [Ciro Santilli's Homepage](split.md)
