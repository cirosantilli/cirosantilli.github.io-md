...
TypeScript is good. It does find errors in your [JavaScript](javascript.md). But it is a form of "turd polishing". But [Ciro Santilli](ciro-santilli-split.md) would rather have a polished turd than a non-polished one.

Part of the reason TypeScript became popular is due to the universality of [asset bundlers](asset-bundler.md). Once you are already using an asset bundler, changing the `.js` extension into `.ts` to get a less shitty experience is an easy choice.
...
But in the end, as with other type of static [linters](linting.md), TypeScript generally transforms a few hard to debug runtime issues, into a bunch of stupid to solve compile time issues, which is generally a good thing.
...
- [typescript/js-from-ts/main.ts](typescript/js-from-ts/main.ts): call [JavaScript](javascript.md) file [typescript/js-from-ts/notmain.js](typescript/js-from-ts/notmain.js) from [TypeScript](typescript-split.md).
  ```
