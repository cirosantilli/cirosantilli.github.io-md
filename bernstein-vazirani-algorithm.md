# Bernstein-Vazirani algorithm

↑ **Parent:** [List of quantum algorithms](list-of-quantum-algorithms.md)  
ⓦ [Wiki](https://en.wikipedia.org/wiki/Bernstein–Vazirani algorithm)

Toy/test/tought experiment algorithm.

## ↑ Ancestors (9)

1. [List of quantum algorithms](list-of-quantum-algorithms.md)
2. [Quantum algorithm](quantum-algorithm.md)
3. [Quantum computing](quantum-computing-split.md)
4. [Quantum information](quantum-information.md)
5. [Information](information.md)
6. [Information technology](information-technology.md)
7. [Area of technology](area-of-technology.md)
8. [Technology](technology-split.md)
9. [Ciro Santilli's Homepage](split.md)

## ← Incoming links (1)

- [IonQ](ionq.md)
