# JOIN (SQL)

↑ **Parent:** [SQL keyword](sql-keyword.md)  
ⓦ [Wiki](https://en.wikipedia.org/wiki/Join_(SQL))

**Table of contents**

- [SQL prefix column names with the table they came from](sql-prefix-column-names-with-the-table-they-came-from.md)
- [INNER JOIN](inner-join.md)
- [OUTER JOIN](outer-join.md)

## ↑ Ancestors (12)

1. [SQL keyword](sql-keyword.md)
2. [SQL](sql-split.md)
3. [Relational database management system](relational-database-management-system.md)
4. [Relational database](relational-database.md)
5. [Type of database](type-of-database.md)
6. [Database](database.md)
7. [Software](software-split.md)
8. [Computer](computer-split.md)
9. [Information technology](information-technology.md)
10. [Area of technology](area-of-technology.md)
11. [Technology](technology-split.md)
12. [Ciro Santilli's Homepage](split.md)

## ← Incoming links (3)

- [Sequelize example](sequelize-example.md)
- [Link to an image or video in another file that has an `\x` on title from another file](sponsor/updates/5/link-to-an-image-or-video-in-another-file-that-has-an-x-on-title-from-another-file.md)
- [SQL example](sql-example.md)
