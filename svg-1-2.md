<h1 id="svg-1-2">SVG 1.2</h1>

↑ **Parent:** [SVG version](svg-version.md)

Dropped in favor of [SVG 2](svg-2.md).

## ↑ Ancestors (10)

1. [SVG version](svg-version.md)
2. [Scalable Vector Graphics](scalable-vector-graphics.md)
3. [Vector graphics](vector-graphics.md)
4. [Image file format](image-file-format.md)
5. [File format](file-format.md)
6. [Computer](computer-split.md)
7. [Information technology](information-technology.md)
8. [Area of technology](area-of-technology.md)
9. [Technology](technology-split.md)
10. [Ciro Santilli's Homepage](split.md)

## ← Incoming links (1)

- [SVG background color](svg-background-color.md)
